...
st.title("Review Analyzer App - Watsonx.ai - v4")
...
with col1:
    ...
    # New fields for WATSONX API KEY and PROJECT ID
    watsonx_api_key = st.text_input("**Enter WATSONX API KEY**", type='password')
    project_id = st.text_input("**Enter PROJECT ID**", type='password')
    
    watsonx_api_key = str(watsonx_api_key).strip()
...
if run:
    # 0. Testing validity of API key and ProjectID for Watsonx.ai service
    try:
        from watsonx_api import checkReview
        checkReview_obj = checkReview(watsonx_api_key, project_id)
        ...
        st.error("Please enter correct WATSONX API KEY and PROJECT ID")
        sys.exit()
    ...
    with col2:
        ...
        # 4. Adding Review star rating Feature (not using LLM)
        with st.spinner('Calculating rating...'):
            print("\n input_text is: ", input_text)
            star_count_dict = helper_functions.count_datapoints_in_review(input_text)  # dict
            
            star_count = star_count_dict['count']  # int
            not_mentioned_datapoints = star_count_dict['not_mentioned_datapoints_str']  # str
            
            print("\n star_count is: ", star_count)
            
            # Displaying the star rating
            star_rating = helper_functions.get_star_rating(star_count)
            custom_subheader("Review Completion Rating", size=16)
            st.markdown(f"**({star_count} out of 5)** : {star_rating}", unsafe_allow_html=True)
            if star_count<5:
                st.write(f"""You can consider writing more about: 
                         {not_mentioned_datapoints}.""")
